...
POLARS_OUT_FILE = f"{HOME_DIR}/scratch/test_out_seq_polars.h5"
PANDAS_OUT_FILE = f"{HOME_DIR}/scratch/test_out_seq_pandas.h5"
NUMPY_OUT_FILE = f"{HOME_DIR}/scratch/test_out_seq_numpy.h5"
...
def run_sort_save_polars():
    ...
    start_time = time.time()
    st_df = pl.DataFrame(st_data).sort("target")
    ...
    comm_ifx.log_at_root(
        _log(), logging.DEBUG,
        f" DATA SORTED {st_df.columns} in {elapsed_time} seconds"
    )
    start_time = time.time()
...
def run_sort_save_numpy():
    comm_ifx = default_comm()
    se_data = SEArgs(out_file_name=NUMPY_OUT_FILE)
    comm_ifx.log_at_root(
        _log(), logging.DEBUG,
        f"MPI #PROC {comm_ifx.size}:: Input:: {se_data.file_name} :: Output :: {se_data.out_file_name}"
    )
    ...
    start_time = time.time()
    st_dtype = np.dtype([
        ('t', st_data['target'].dtype), ('s', st_data['target'].dtype)
    ])
    st_array = np.fromiter(
        zip(st_data['target'], st_data['source']),
        dtype=st_dtype
    )
    st_array.sort()
    tgt_col = st_array['t'] 
    ...
    comm_ifx.log_at_root(
        _log(), logging.DEBUG,
        f" DATA SORTED {st_array.shape} {tgt_col.shape} {src_col.shape} in {elapsed_time} seconds"
    )
    start_time = time.time()
    write_data(se_data.out_file_name, tgt_col, src_col)
    elapsed_time = time.time() - start_time
...
def test_sort_save(method: t.Literal["polars", "pandas", "numpy"]):
    comm_ifx = default_comm()
    if comm_ifx.rank == 0:
